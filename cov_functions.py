...
#-----------------------------------------------------------------
    ...
    ##
def poly_solv_ode(t,y,beta_L_factor=1,beta_H_factor=1,t_control=None,vaccine_time=None,ICU_grow=0,let_HR_out=True):
    ##
    ...
    #--------------------------------------------------------------------
    ##
def poly_calc_ode(I0,R0,H0,C0,D0,beta_L_factor,beta_H_factor,t_control,T_stop,vaccine_time,ICU_grow,let_HR_out): # critical,death
    ...
    #--------------------------------------------------------------------
    ##
def run_model(beta_L_factor=1,beta_H_factor=1,t_control=None,T_stop=None,vaccine_time=None,I0=None,R0=None,H0=None,C0=None,D0=None,ICU_grow=0,let_HR_out=True):
    try:
        y_out, tim = poly_calc_ode(I0=I0,R0=R0,H0=H0,C0=C0,D0=D0,beta_L_factor=beta_L_factor,beta_H_factor=beta_H_factor,t_control=t_control,T_stop=T_stop,vaccine_time=vaccine_time,ICU_grow=ICU_grow,let_HR_out=let_HR_out) # critical=critical,death=death,
    except Exception as e:
        print('running model error: ',e)
        tim = np.linspace(0,T_stop, 365)
        y_out = np.zeros((12,len(tim)))
